...
<head>
    ...
    <title>On Process - GSM Pioneer</title>
...
function fetchRecord() {
    ...
    var dataTable1 = $('#tbl_health').DataTable({
        ...
        "ajax": {
            url: "functions/select_onprocess.php?",
            type: "POST",
            cache: false,
...
$(document).on("click", "#btn_void_process", function() {
    var transaction_num = $(this).data('transaction_num');
    var stall_id = $(this).data('stall_id');
    var other_data = 'transaction_num=' + transaction_num + '&stall_id=' + stall_id;


    swal({
        title: "Continue?",
        text: "Do you want to void this appointment?",
        type: "question",
        ...
        if (result.value) {
            $.ajax({
                ...
                url: "functions/void_process.php?" + other_data,
                type: "POST",
                contentType: false,
                cache: false,
                processData: false,
                ...
                success: function(data) {
                    ...
                    if (data.includes('200')) {
                        ...
                        swal({
                            title: "Success!",
                            text: "Appointment has been voided successfully!",
                            type: "success"
                        }).then(okay => {
                            if (okay) {
