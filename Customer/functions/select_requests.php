...
$query = "SELECT `_id`, `_transaction_num`, `_date`, `_time`, `_customer_email`, `_customer_name`, `_customer_contact`, `_customer_address`, `_origin`, `_stall_id`, `_status` FROM `tbl_appointment` WHERE `_customer_email` LIKE '$email' AND `_status` LIKE 'pending'";
...
while($row = mysqli_fetch_array($result)) {
 ...
 $sub_array[] = '<div class="btn-group" id="td_name'.$row["_id"].'">
		    <a data-toggle="modal" data-target="#concernModal" style="color:white;" class="btn btn-dark" id="btn_concern" 
		    data-id="'.$row["_id"].'"
		    data-transaction_num="'.$row["_transaction_num"].'"
		    data-stall_id="'.$row["_stall_id"].'"
		    >Add concern</a>
		    <a  data-toggle="modal" data-target="#concernTableModal" style="color:white;" class="btn btn-primary"
		    data-transaction_num="'.$row["_transaction_num"].'"	
		    data-stall_id="'.$row["_stall_id"].'"	
		     id="btn_view" 
		   
		    >View concern</a>
		    <a data-toggle="modal" data-target="#cancelModal" style="color:white;" class="btn btn-danger" id="btn_cancel" data-id="'.$row["_id"].'"
