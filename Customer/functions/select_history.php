...
$query = "SELECT `_id`, `_transaction_num`, `_date`, `_time`, `_customer_email`, `_customer_name`, `_customer_contact`, `_customer_address`, `_origin`, `_stall_id`, `_status`,`_cancel_reason`,`_rating`,`_feedback`,`_date_feedback` FROM `tbl_appointment` WHERE `_customer_email` LIKE '$email' AND `_status` LIKE 'completed'";
...
while($row = mysqli_fetch_array($result)) {
 ...
 $sql = "SELECT `_service`,`_device_unit`,`_qty`,`_total_amount`,`_emp_name` FROM `tbl_service_rendered` WHERE `_transaction_num` LIKE '$tn' AND `_status` LIKE 'completed'";
 ...
 $sub_array[] = '<div class="btn-group" id="td_name'.$row["_id"].'">
		    ...
		    >View Services</a>
		    <a  data-toggle="modal" data-target="#feedbackModal" style="color:white;" class="btn btn-primary"
		    data-transaction_num="'.$row["_transaction_num"].'"	
		     id="btn_feedback" 
...
require ("../../db_connection/myConn.php");
$email=$_SESSION["_email"];
 $query = "SELECT `_id`, `_transaction_num`, `_date`, `_time`, `_customer_email`, `_customer_name`, `_customer_contact`, `_customer_address`, `_origin`, `_stall_id`, `_status` FROM `tbl_appointment` WHERE `_customer_email` LIKE '$email' AND `_status` LIKE 'completed'";
