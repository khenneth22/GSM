...
$sql2 = "SELECT `_item_code`,`_qty`,`_stall_id` FROM `tbl_service_rendered` WHERE `_transaction_num` LIKE '$transaction_num' AND `_service_type` LIKE 'Hardware'";
	...
	if($result2->num_rows > 0) {
		...
		while($row2 = $result2->fetch_assoc()) {
			...
			$sql = "SELECT `_item_code`,`_quantity` FROM `tbl_items` WHERE `_item_code` LIKE '$item_code' AND `_stall_id` LIKE '$stall_id'";
				...
				if($result->num_rows > 0) {
					...
					while($row = $result->fetch_assoc()) {
						...
						if ($currentqty<$qty) {
							echo 'Insufficient stocks: '.$item_code;
							return;
						}
					}
				}
...
$now = date_format($dt,"Y-m-d H:i:s");
	...
	$query="UPDATE `tbl_appointment` SET `_status` ='on process', `_date_processed` = '$now' WHERE `_transaction_num` LIKE '$transaction_num'";
					if(mysqli_query($con_str,$query)){

							$query="UPDATE `tbl_service_rendered` SET `_status` ='on process', `_date_processed` = '$now' WHERE `_transaction_num` LIKE '$transaction_num'";
										if(mysqli_query($con_str,$query)){
											echo '200';	
										}
								...
								$sql = "SELECT `_item_code`,`_quantity`,`_item`,`description`,`_price` FROM `tbl_items` WHERE `_item_code` LIKE '$item_code' AND `_stall_id` LIKE '$stall_id'";
									...
									if($result->num_rows > 0) {
										...
										while($row = $result->fetch_assoc()) {
											...
											$query="INSERT INTO `tbl_inventory`(`_date`,`_item_code`, `_item`, `description`, `_price`, `_stall_id`, `_beginning`,`_out`, `_ending`, `_user`) VALUES ('$now','$item_code','$item_name','$description',$price,'$stall_id',$currentqty,$qty,$ending_qty,'$emp_name')";
											if(mysqli_query($con_str,$query)){
