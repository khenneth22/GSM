...
$sql = "SELECT `_customer_email` FROM `tbl_appointment`  WHERE  `_transaction_num` LIKE '$transaction_num'";
  ...
  if($result->num_rows > 0) {
	...
	$query="UPDATE `tbl_appointment` SET `_status` ='approved',`_emp_email` ='$email',`_emp_name` = '$emp_name' WHERE `_transaction_num` LIKE '$transaction_num'";
					if(mysqli_query($con_str,$query)){

							$query="UPDATE `tbl_service_rendered` SET `_status` ='approved',`_emp_email` ='$email',`_emp_name` = '$emp_name' WHERE `_transaction_num` LIKE '$transaction_num'";
										if(mysqli_query($con_str,$query)){
											echo '200';	
										}
